    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            for (i = 0; i < responsedata.length; i++) {
                ...
                $('#answeredlist').append('<li style="font-weight: bold">'+responsedata[i][0]+' - '+responsedata[i][1]+'</li>').listview('refresh');
                ...
                $.ajax({
                    ...
                    async: false,
                    url: SoNM.Settings.helperUrl,
                    data: "function=answered_getquestions" + "&unit=" + responsedata[i][0] + "&user=" + session.sessionId,
                    success: function (resp2) {
                        var responsedata2 = $.parseJSON(resp2);

                        for (j = 0; j < responsedata2.length; j++) {
                            /* add list item */
                            $('#answeredlist').append('<li><a href="viewquestion.html?id='+responsedata2[j][0]+'" rel="external">'+responsedata2[j][1]+'</a></li>').listview('refresh');
                        }
                    }
                });
